/**
  ORIGINAL POST-PROCESSOR CREATED BY AUTODESK

  Copyright (C) 2012-2016 by Autodesk, Inc.
  All rights reserved.

  Boxford post processor configuration.

  $Revision: 41369 65a1f6cb57e3c7389dc895ea10958fc2f7947b0d $
  $Date: 2017-03-20 14:12:44 $
  
  FORKID {D02700C4-2920-4c4e-B621-E7B1C5E1EE67}
*/

/**

  MODIFICATIONS FOR USE WITH VMC 260 MADE BY READING HACKSPACE
  
  Use at own risk having first ascertained that they will not harm you or your machine

*/


description = "Boxford VMC 260";
vendor = "BOXFORD";
vendorUrl = "http://rlab.org.uk/wiki/Boxford_260_VMC";
legal = "";
certificationLevel = 2;
minimumRevision = 24000;

longDescription = "Milling post for Boxford VMC 260.";
...
function onOpen() {
  ...
  // Custom header output for 260VMC
  writeln("[GM HEADER]");
  writeln("type=mill");
  ...
  if(isWorkpieceDefined()) {
    var SizeX = getWorkpiece().upper.getX()-getWorkpiece().lower.getX();
    var SizeY = getWorkpiece().upper.getY()-getWorkpiece().lower.getY();
    var SizeZ = getWorkpiece().upper.getZ()-getWorkpiece().lower.getZ();
    writeln("bar size=" + (Math.round(SizeX*1000)/1000) +"," + (Math.round(SizeY*1000)/1000) +"," + (Math.round(SizeZ*1000)/1000));     
  } else {
    warning("Default bar size placed in file.");
    writeln("bar size=100.0000,100.0000,20.0000");  
  }

  writeln("bar shape=BOX");
  writeln("");
